...
| `BIN-C2` | For every binary $`2 \times 2`$ law, $`T(p) \le 2\,\tau(p)`$; on full support the constant code or one of the four singleton codes attains $`D_p(g) \le 2\,\tau(p)`$. | `paper proof` ([Theorems 6.1 and 6.2](binary-factor-two.md#6-assembly)). The inequality alone, over the same upstream `T`, `tau`, and `IsPMF`, was `kernel-verified` in the reviewed source workspace (2026-09-06); the full-support witness clause was not. | Proved from `BIN-CHORD-CUT`, `BIN-CENTER`, and `BIN-FIXED-CUT` on full support, then transferred to every law by the kernel-verified `T_le_mul_tau_of_forall_fullSupport` at constant two. **Of those three gates, one is `kernel-verified` in this repository and two are not:** `BIN-FIXED-CUT` is verified here in full, while `BIN-CHORD-CUT` and `BIN-CENTER` remain `paper proof` with named parts of each verified here, as their own rows record. No gate is complete at any law. No Lean declaration in this repository states it, though the assembly from the three gates is `kernel-verified` here under `BIN-CHORD-CUT`; the identities and fixed logarithm comparisons of the page are replayed by `scripts/check_factor_two_identities.py`, which is not proof evidence. Sharpness of the constant is not claimed, and `BIN-C9` remains the only binary constant kernel-verified here. |
